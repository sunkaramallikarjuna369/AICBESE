...
class ReplicateAIProvider(AIProvider):
    """
    AI Provider using Replicate API for cost-effective AI inference.
    
    Replicate offers pay-per-use pricing with no minimum charges, making it
    ideal for low-budget deployments. Uses open-source models like Llama 3.1.
    
    Cost comparison:
    - Vertex AI Gemini: ~$0.0005-0.002 per 1K characters
    - Replicate Llama 3.1 8B: ~$0.05 per million tokens (~$0.00005 per 1K chars)
    
    Environment variables:
    - REPLICATE_API_TOKEN: Your Replicate API token
    - REPLICATE_MODEL: Model to use (default: meta/meta-llama-3.1-8b-instruct)
    """
    
    def __init__(self, api_token: str, model: str = "meta/meta-llama-3.1-8b-instruct"):
...
IMPORTANT: Return ONLY valid JSON, no markdown formatting or explanation."""
        ...
        try:
            response_text = await self._generate_content(prompt, max_tokens=2000)
            ...
            logger.error(f"Replicate generation failed, falling back to mock: {e}")
...
- narrative: 2-3 paragraph explanation (string)
- real_world_scenario: specific scenario description (string)
- daily_life_examples: list of 5 examples (array of strings)
- industry_applications: list of 4 applications (array of strings)
- career_connections: list of 4 careers (array of strings)
- mini_project_ideas: list of 4 projects (array of strings)
...
IMPORTANT: Return ONLY valid JSON, no markdown formatting or explanation."""
        ...
        try:
            ...
            logger.error(f"Replicate RWAL generation failed, falling back to mock: {e}")
            mock_provider = MockAIProvider()
...
- question_type: multiple_choice or short_answer (string)
- options: list of 4 options for multiple choice (array of strings)
- correct_answer: the correct answer (string)
- explanation: detailed explanation (string)
- hint: helpful hint (string)
- bloom_level: {request.bloom_level} (string)
...
IMPORTANT: Return ONLY valid JSON array, no markdown formatting or explanation."""
        ...
        try:
            ...
            logger.error(f"Replicate quiz generation failed, falling back to mock: {e}")
            mock_provider = MockAIProvider()
...
- steps: array of step objects, each with: step_number (int), action (string), explanation (string), visual_hint (string)
- final_answer: the answer (string)
- real_world_connection: connection to real world (string, for dull mode)
- verification: how to verify the answer (string, for average/clever mode)
...
IMPORTANT: Return ONLY valid JSON, no markdown formatting or explanation."""
        ...
        try:
            ...
            logger.error(f"Replicate worked example generation failed, falling back to mock: {e}")
            mock_provider = MockAIProvider()
    ...
    async def generate_text(
        ...
        """Generate text response for RAG and general queries using Replicate."""
        try:
            response_text = await self._generate_content(prompt, max_tokens)
            return response_text
        except Exception as e:
            logger.error(f"Replicate text generation failed, falling back to mock: {e}")
            mock_provider = MockAIProvider()
...
def get_ai_provider() -> AIProvider:
    ...
    Supported providers:
    - mock: Free, no API calls, returns pre-defined responses (default)
    - replicate: Low-cost, pay-per-use via Replicate API (recommended for low budget)
    - vertex: Google Vertex AI with Gemini (higher cost, requires GCP project)
    
    Environment variables:
    - AI_PROVIDER: Provider type (mock, replicate, vertex)
    - REPLICATE_API_TOKEN: Required for replicate provider
    - REPLICATE_MODEL: Optional, defaults to meta/meta-llama-3.1-8b-instruct
    - VERTEX_AI_PROJECT: Required for vertex provider
    ...
    if provider_type == "replicate":
        ...
        model = os.getenv("REPLICATE_MODEL", "meta/meta-llama-3.1-8b-instruct")
        
        if not api_token:
            logger.warning("REPLICATE_API_TOKEN not set, falling back to mock provider")
            return MockAIProvider()
        
        try:
            return ReplicateAIProvider(api_token, model)
        except Exception as e:
            logger.error(f"Failed to create Replicate provider: {e}, falling back to mock")
            return MockAIProvider()
